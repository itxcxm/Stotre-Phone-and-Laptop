...
const Update = () =>{
    ...
    const getProcductDeyails= async()=>{
        let result = await axios.get(`http://localhost:5000/update/${params.slug}`)
        ...
        setCpu(result.data.specifications.cpu);
        setRam(result.data.specifications.ram);
        setStorage(result.data.specifications.storage);
        setScreen(result.data.specifications.screen);
        setGpu(result.data.specifications.gpu);
        setBattery(result.data.specifications.battery);
        setOs(result.data.specifications.os);
        setProcessor(result.data.specifications.processor);
        setCamera(result.data.specifications.camera);
    ...
    const update = async () => {
        ...
        try {
            const response = await axios.put(
                `http://localhost:5000/update/${params.slug}`,
                {
            ...
            alert(`Lỗi cập nhật sản phẩm: ${error.response.data.message}`);
        }
    }
    ...
    return(
        <div className="flex h-full w-full justify-center items-center bg-gray-200">
            ...
            <button onClick={update} type="submit" className="w-full bg-blue-500 text-white p-4 mt-3 rounded hover:bg-blue-600">Lưu sản phẩm</button>
            </div>
        </div>
    )
}
